...
def build_parser() -> argparse.ArgumentParser:
    ...
    copywriting.add_argument(
        ...
        choices=("짧게", "보통", "긴"),
        default="보통",
        help="문구/해시태그 분량 프리셋 (UI의 '글 길이' 값).",
    )
    ...
    info.add_argument(
        "--promo-target",
        default="",
        help="홍보 대상 (예: '20-30대 직장인', '반려동물 보호자'). target-customer와 별개로 홍보 캠페인이 겨냥하는 대상을 명시할 때 사용.",
    )
    ...
    info.add_argument(
        "--draft-copy",
        default="",
        help="사장님이 직접 써본 홍보 문구 초안. 있으면 이를 참고/다듬어서 최종 카피를 생성.",
    )
    info.add_argument(
        "--info-output",
